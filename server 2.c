...
int saveDns(char user, char pid)
{
    // abrir o arquivo dns.txt e salvar o user com o pid na frente
    // se o arquivo não existir, criar
    // se o arquivo existir, adicionar o user com o pid na frente
    // se o user já existir, atualizar o pid
...
int getCredentials(char user, char password)
{
    FILE *fp;
...
int logginInterno(char user, char password)
{
    printf("Tentativa de loggin de %s %s", user, password);

    if (getCredentials(user, password) == 0)
    {
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    { // main accept() loop
        ...
        // Verificar o argv[2] e argv[3] para fazer o loggin e conectar.
        ...
        if (logginInterno(argv[2], argv[3]) == 0)
        {
            // use a função de salvar dns passando o argv[2] e o pid do processo
            // saveDns(argv[2], new_fd);
